...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.iotReading.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.iot-readings.update", [$iotReading->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('responden') ? 'is-invalid' : '' }}" name="responden_id" id="responden_id" required>
                    @foreach($respondens as $id => $entry)
                        <option value="{{ $id }}" {{ (old('responden_id') ? old('responden_id') : $iotReading->responden->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fetal_hr') ? 'is-invalid' : '' }}" type="number" name="fetal_hr" id="fetal_hr" value="{{ old('fetal_hr', $iotReading->fetal_hr) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resp_count') ? 'is-invalid' : '' }}" type="number" name="resp_count" id="resp_count" value="{{ old('resp_count', $iotReading->resp_count) }}" step="1" required>
