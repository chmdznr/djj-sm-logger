...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.responden.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.respondens.update", [$responden->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.responden.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $responden->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $responden->kode) }}" required>
                @if($errors->has('kode'))
                    <div class="invalid-feedback">
                        {{ $errors->first('kode') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('usia') ? 'is-invalid' : '' }}" type="number" name="usia" id="usia" value="{{ old('usia', $responden->usia) }}" step="1" required>
                @if($errors->has('usia'))
                    <div class="invalid-feedback">
                        {{ $errors->first('usia') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Responden::HIS_ADEKUAT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('his_adekuat') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="his_adekuat_{{ $key }}" name="his_adekuat" value="{{ $key }}" {{ old('his_adekuat', $responden->his_adekuat) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Responden::PERGERAKAN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('pergerakan') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="pergerakan_{{ $key }}" name="pergerakan" value="{{ $key }}" {{ old('pergerakan', $responden->pergerakan) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paritas') ? 'is-invalid' : '' }}" type="number" name="paritas" id="paritas" value="{{ old('paritas', $responden->paritas) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kardiotokografi') ? 'is-invalid' : '' }}" type="number" name="kardiotokografi" id="kardiotokografi" value="{{ old('kardiotokografi', $responden->kardiotokografi) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('alamat') ? 'is-invalid' : '' }}" name="alamat" id="alamat">{!! old('alamat', $responden->alamat) !!}</textarea>
