...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.smReading.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sm-readings.update", [$smReading->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('responden') ? 'is-invalid' : '' }}" name="responden_id" id="responden_id" required>
                    @foreach($respondens as $id => $entry)
                        <option value="{{ $id }}" {{ (old('responden_id') ? old('responden_id') : $smReading->responden->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('spo_2') ? 'is-invalid' : '' }}" type="number" name="spo_2" id="spo_2" value="{{ old('spo_2', $smReading->spo_2) }}" step="0.01" required>
                @if($errors->has('spo_2'))
                    <div class="invalid-feedback">
                        {{ $errors->first('spo_2') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hr') ? 'is-invalid' : '' }}" type="number" name="hr" id="hr" value="{{ old('hr', $smReading->hr) }}" step="1" required>
                @if($errors->has('hr'))
                    <div class="invalid-feedback">
                        {{ $errors->first('hr') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('skin_temp') ? 'is-invalid' : '' }}" type="number" name="skin_temp" id="skin_temp" value="{{ old('skin_temp', $smReading->skin_temp) }}" step="0.01" required>
